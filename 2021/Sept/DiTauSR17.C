...
///////////// Signal Region ////////////////////////////
	...
	TFile *SR1 = new TFile("DY+Jets_17F_DITAU_ST.root");
		TH1F *DY = (TH1F*)SR1->Get("NDiJetCombinations/DiTauReconstructableMassDeltaPt");
			DY->SetLineColor(1);
	...
	TFile *SR2 = new TFile("SingleTop.root");
		TH1F *ST = (TH1F*)SR2->Get("NDiJetCombinations/DiTauReconstructableMassDeltaPt");
			ST->SetLineColor(1);
			ST->SetFillColor(8);	
	...
	TFile *SR3 = new TFile("tbar{t}.root");
		TH1F *TT = (TH1F*)SR3->Get("NDiJetCombinations/DiTauReconstructableMassDeltaPt");
			TT->SetLineColor(1);
	...
	TFile *SR4 = new TFile("VV.root");
		TH1F *VV = (TH1F*)SR4->Get("NDiJetCombinations/DiTauReconstructableMassDeltaPt");
			VV->SetLineColor(1);
			VV->SetFillColor(46);


	TFile *SR5 = new TFile("W+Jets.root");
		TH1F *WJ = (TH1F*)SR5->Get("NDiJetCombinations/DiTauReconstructableMassDeltaPt");
			WJ->SetLineColor(1);
			WJ->SetFillColor(kAzure+6);	
   TFile *SR6 = new TFile("QCD2017.root");
		TH1F *QCD = (TH1F*)SR6->Get("B_qcd");
		   QCD->SetLineColor(1);
  ...
  TFile *S_0  = new TFile("Zp500_glh1p0.root");
  TFile *S_1  = new TFile("Zp2500_glh1p0.root");
  ...
  TH1F *SIG2 =(TH1F*)sig2->Rebin (n, "ReconstructedMass", xbins);
   ...
   TH1F *qcd = (TH1F*)QCD->Rebin (n, "ReconstructedMass", xbins);
		qcd->Sumw2();



 std::cout<<qcd->Integral()<<std::endl;
...
TCanvas *c0 = new TCanvas("c1","rm_nn",550,500);
  ...
  c0->cd();
	...
	h->Draw();
		h->SetTitle(";m_{rec} (#tau, #tau, #Delta p_{T}) [GeV] ; Events");
