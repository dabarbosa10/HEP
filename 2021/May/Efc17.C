{
  ...
  //gStyle->SetOptFit(0);
   ...
   TFile *f0  = new TFile("Zprime_NonUniversalSSM_M250_gl1p0_gh1p0.root");
   TFile *f1  = new TFile("Zprime_NonUniversalSSM_M500_gl1p0_gh1p0.root");
   TFile *f2  = new TFile("Zprime_NonUniversalSSM_M750_gl1p0_gh1p0.root");
   TFile *f3  = new TFile("Zprime_NonUniversalSSM_M1000_gl1p0_gh1p0.root");
   TFile *f4  = new TFile("Zprime_NonUniversalSSM_M1250_gl1p0_gh1p0.root");
   TFile *f5  = new TFile("Zprime_NonUniversalSSM_M1500_gl1p0_gh1p0.root");
   TFile *f6  = new TFile("Zprime_NonUniversalSSM_M1750_gl1p0_gh1p0.root");
   TFile *f7  = new TFile("Zprime_NonUniversalSSM_M2000_gl1p0_gh1p0.root");
   TFile *f8  = new TFile("Zprime_NonUniversalSSM_M2250_gl1p0_gh1p0.root");
   TFile *f9  = new TFile("Zprime_NonUniversalSSM_M2500_gl1p0_gh1p0.root");
   TFile *f10  = new TFile("Zprime_NonUniversalSSM_M2750_gl1p0_gh1p0.root");
   TFile *f11 = new TFile("Zprime_NonUniversalSSM_M3000_gl1p0_gh1p0.root");
   TFile *f12 = new TFile("Zprime_NonUniversalSSM_M3500_gl1p0_gh1p0.root");
   TFile *f13 = new TFile("Zprime_NonUniversalSSM_M4000_gl1p0_gh1p0.root");
   ...
   TH1F*hTPt12=(TH1F*)f10->Get("NDiJetCombinations/Events");
   TH1F*hTPt13=(TH1F*)f11->Get("NDiJetCombinations/Events");
   ...
   double_t af10, be10, ra10, af11, be11, ra11, af12, be12, ra12, af13, be13, ra13 ;
   ...
   be13=hTPt13->GetBinContent(1);
   af13=hTPt13->GetBinContent(2);
   ra13=af13/be13;
   er13= TMath::Sqrt( (1.0/be13) + (1.0/af13) );
   er13=(er13*(ra13))*100;

   double xs250, xs500, xs750, xs1000, xs1250, xs1500, xs1750, xs2000, xs2500, xs3000, xs3500, xs4000;

   xs250=62.9*1000.0;
   xs500=6.041*1000.0;
   xs750=1.367*1000.0;
   xs1000=0.4422*1000.0;
   xs1250=0.1737*1000.0;
   xs1500=0.07673*1000.0;
   xs1750= 0.03694*1000.0;
   xs2000=0.01864*1000.0;
   xs2500=0.005381*1000.0;
   xs3000=0.001719*1000.0;
   xs3500= 0.0005883*1000.0;
   xs4000= 0.0002093*1000.0;
...
/*
   ...
   cout<<"2500 & "<<ra8*100<<"$ \\pm$ "<<er8<<" & " << be8 <<" & " << af8 << " & " << xs2500 <<" & "<<(ra8)*lumi*xs2500 <<" \\\\"<<endl;
   cout<<"3000 & "<<ra9*100<<"$ \\pm $"<<er9<<" & " << be9 <<" & " << af9 << " & " << xs3000 <<" & "<<(ra9)*lumi*xs3000 <<" \\\\"<<endl;
   cout<<"3500 & "<<ra10*100<<"$ \\pm$ "<<er10<<" & " << be10 <<" & " << af10 << " & "<< xs3500 <<" & "<<(ra10)*lumi*xs3500 <<" \\\\"<<endl;
   cout<<"4000 & "<<ra11*100<<"$ \\pm $"<<er11<<" & " << be11 <<" & " << af11 << " & " << xs4000 <<" & "<<(ra11)*lumi*xs4000 <<" \\\\"<<endl;
*/
   cout<<"\\hline\\hline"<<endl;
   cout<<"Mass(GeV)   & Efficiency  \\%     \\\\"<<endl;
   cout<<"250 & "<<ra0*100<<"$ \\pm $"<<er0<<" "<<endl;
   cout<<"500 & "<<ra1*100<<"$ \\pm $"<<er1<<" "<<endl;
   cout<<"750 & "<<ra2*100<<"$ \\pm $"<<er2<<" "<<endl;
   cout<<"1000 & "<<ra3*100<<"$ \\pm $"<<er3<<" "<<endl;
   cout<<"1250 & "<<ra4*100<<"$ \\pm $"<<er4<<" "<<endl;
   cout<<"1500 & "<<ra5*100<<"$ \\pm $"<<er5<<" "<<endl;
   cout<<"1750 & "<<ra6*100<<"$ \\pm $"<<er6<<" "<<endl;
   cout<<"2000 & "<<ra7*100<<"$ \\pm $"<<er7<<" "<<endl;
   cout<<"2250 & "<<ra8*100<<"$ \\pm $"<<er8<<" "<<endl;
   cout<<"2500 & "<<ra9*100<<"$ \\pm $"<<er9<<" "<<endl;
   cout<<"2750 & "<<ra10*100<<"$ \\pm $"<<er10<<" "<<endl;
   cout<<"3000 & "<<ra11*100<<"$ \\pm $"<<er11<<" "<<endl;
   cout<<"3500 & "<<ra12*100<<"$ \\pm $"<<er12<<" "<<endl;
   cout<<"4000 & "<<ra13*100<<"$ \\pm $"<<er13<<" "<<endl;
